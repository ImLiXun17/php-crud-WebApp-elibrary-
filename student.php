...
<body>
    ...
    if(!isset($_GET['page'])){
        ...
        //create query
        $query = "SELECT * FROM student inner join college on student.college_id = college.id ORDER BY student_name ASC LIMIT  $first_page_number,$result_per_page";
        if(isset($_GET['submit'])){
           $search = $_GET['search'];
           if (strlen($search) > 0 ){
               $query = "SELECT * FROM student inner join college on  student.college_id = college.id WHERE sid = '$search' or college_name = '$search' or address LIKE '%".$search."%' or student_name LIKE '%".$search."%' 
               ORDER BY student_name ASC";
            }
            else{
              $query  = "SELECT * FROM student inner join college on  student.college_id = college.id ORDER BY student_name ASC";
            }
           }
        //get the result
                ...
                <div class="container-fluid">
                                    ...
                                    <table class="table table-hover table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo $student['corporate_email'];?></td>
                                                <td><?php echo $student['college_name'];?></td>
                                                <td><?php echo $student['address'];?></td>
                                                <td>
                                                    <a href="student-edit.php?sid=<?php echo $student['sid']; ?>">
                                                    <button type="submit" class ="btn btn-warning btn-fill pull-right">Edit</button>
                                                    </a>
                                                </td>
                                                <td>
                                                <a href="student-delete.php?sid=<?php echo $student['sid']; ?>">
                    ...
                    <?php 
                                for($page=1; $page <= $number_of_page; $page++){
                                    echo '<a href="student.php?page='.$page.'" style="margin: right 5px; padding: 5px;">'. $page. ' </a>';
                                }
                            ?>
                </div>
            </div>
